...
function [predTr, predTe] = knn_classify_bilin_diag(dataTr,labelTr,dataTe,M,k)
	...
	for i=1:B:max(nTr,nTe)
		if i <= nTr
			BTr = min(B-1,nTr-i);
			DTr = dataTr*(bsxfun(@times,dataTr(i:i+BTr,:)',M));
		...
		if i <= nTe
			BTe = min(B-1,nTe-i);
            DTe = dataTr*(bsxfun(@times,dataTe(i:i+BTe,:)',M));
